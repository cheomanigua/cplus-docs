...
* **Where it is implemented:** Inside `Systems.hpp` / `Systems.cpp`, you define the `IGameCommand` interface and the `AttackCommand` struct.
* **How it works in your project:** Instead of the `main.cpp` file directly telling the `CombatSystem` to deal damage, it instantiates an `AttackCommand` object. This object packages together the executioner, the target, the combat subsystem reference, and the specific skill payload into a clean, detached unit of work. When `controller.IssueCommand(npcEntity, attackCmd)` is invoked, the command executes its encapsulated payload.
...
* **Where it is implemented:** Inside `Systems.hpp` / `Systems.cpp`, you define the `IEntityFactory` interface and implement the concrete `RpgEntityFactory` class.
...
* **Model (The Components & Registry):** Your data layer is represented by the raw component structs inside `Core.hpp` (`StatsComponent`, `IdentityComponent`, `SkillsComponent`) and the data pools held by the factory array grids.
* **View (`CharacterConsoleView`):** Referenced inside your namespace as a rendering utility. It reads continuous state parameters using `std::span` (C++20) or pointer-size segments and prints them cleanly to the console terminal without altering or owning the components.
...
* **Where it is in your code:** In `Core.hpp`, `struct Entity` contains nothing but a single integer: `int Id;`. It acts simply as a row index pointer to look up data in the component arrays.
...
* **Where it is in your code:** Inside `Core.hpp`, you have defined pure data containers like `StatsComponent` (holding `Health` and `Mana`) and `SkillsComponent` (holding a bitmask integer).
...
* **Where it is in your code:** Your `CombatSystem` inside `Systems.cpp` is a textbook ECS system. When an attack triggers, it accepts an entity handle, looks up its raw positions in the component array arrays via its `Id`, and directly mutates the values inside the `StatsPool`.
...
```cpp
IdentityComponent IdentityPool[100];
StatsComponent StatsPool[100];
int EquipmentPool[100];
...
1. You need a standard crew member, so you inherit: `class CrewMember : public NPC`.
2. You need an automated cleaning droid that moves around the station, so you inherit: `class StationDroid : public NPC`.
3. You need a security guard who can attack a criminal: `class SecurityGuard : public CrewMember`.
...
IdentityPool[alice] = { CharacterClass::Rogue };
StatsPool[alice]    = { 100, 0 };
SkillsPool[alice]   = { SkillDatabase::GetSingleBitmask("Illusion") };

// 2. A peaceful cleaning droid
int wallE = CreateEntityID();
DroidMovementPool[wallE] = { 100, 2.0f };
...
DroidMovementPool[terminator] = { 500, 5.0f };
StatsPool[terminator]         = { 300, 0 };
EquipmentPool[terminator]     = HashString("assassins_dagger"); // Give the robot a blade!
...
### 1. POD (Plain Old Data) `struct`s

* **Have they been used?** **Yes, extensively.** All your components (`IdentityComponent`, `StatsComponent`, `SkillsComponent`) and execution payloads (`AttackCommand`) are explicitly declared as `struct`s containing only data.

#### Why it matters for your genres:

In a standard object-oriented game, every NPC or Ship is an instance of a `class`. Classes often live on the **Heap**, meaning memory is scattered everywhere. To access them, the CPU has to "chase pointers," which stalls the processor and triggers cache misses.

Because your components are POD `structs`, they possess no object overhead (no vtable pointers). When you put them inside an array in your factory:
...
```cpp
void ProcessAttack(const Entity& attacker, const Entity& target, const std::string& skillId);
StatsComponent& targetStats = registry->StatsPool[target.Id];
view.RenderCharacterSheet(attacker, identities, stats, skills);

```

#### What this does under the hood:

Normally, when you pass a large `struct` by value, C++ copies the entire structure into a new memory location. If a struct is large, copying it 60 times a second creates massive, unnecessary data-shuffling overhead.

* **`const T&` (Read-Only Reference):** By using `const Entity& attacker`, you tell C++ to pass a tiny memory address to the existing struct instead of copying it. The `const` keyword guarantees that the system is forbidden from accidentally altering the attacker's ID.
* **`T&` (Read-Write Reference):** By using `StatsComponent& targetStats = registry->StatsPool[target.Id];`, you are pulling a live wire directly out of your component array. When you modify health via `targetStats.Health -= finalDamage;`, you are writing directly into the array block in memory. There is zero temporary copying required.

### 3. `std::span` (C++20)

* **Have they been used?** **Yes, inside your view logic.** Before rendering your game state, you capture your raw pools as views of memory:

```cpp
std::span<IdentityComponent> identities(registry->IdentityPool);
std::span<StatsComponent> stats(registry->StatsPool);
std::span<SkillsComponent> skills(registry->SkillsPool);

```

#### Why this is a masterclass move:

A `std::span<T>` is a non-owning, contiguous representation of arbitrary memory. It provides the same benefits as a raw pointer/size pair but with the safety of modern C++. It allows your view layer to operate on your data arrays without the overhead of copying, and because it is non-owning, it cannot accidentally trigger deletion or reallocation of your core component buffers.

### 4. `alignas` and Data Packing

* **Have they been used?** **Not yet, but they are your secret weapon for the genres you want to build.**

`alignas(n)` allows you to ensure your components are aligned to cache-line boundaries, while `#pragma pack(push, 1)` or explicit ordering allows you to minimize padding. This is critical for cache-friendly memory layouts.

#### How you will use it for your upcoming games:

##### For your Crime/NPC Simulation (Memory-Packing Needs):

In C++, you can use `union` structures to achieve the exact same memory-saving goals as Explicit Layouts in C#:

```cpp
struct ActionDataComponent {
    union {
        int Sleeping_HoursRested;
        int Investigating_ClueID;
        int Socializing_TargetNPCID;
    } data;
    float RoutineTimer;
};
...
Here, all three `int` fields share the exact same 4 bytes of memory. Because an NPC can never be asleep, investigating a clue, and actively talking to someone at the exact same millisecond, you compress your component memory footprint significantly.

##### For your Harpoon Simulation (Binary Network/Save Data Parsing):

When parsing technical sensor packets or saving military data arrays, C++ allows you to `reinterpret_cast` raw byte buffers directly into your structs (provided they are PODs), allowing you to process incoming network data as native game structures without needing to call any deserialization functions.
...
| --- | --- | --- |
| **POD `struct**` | All components (`Core.hpp`) | Eliminates Object Overhead & Cache Misses |
