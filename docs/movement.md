...
struct MovementBuffers
{
    ...
    Vector2 LastPositions[EngineConfig::MaxEntities];
    float Speeds[EngineConfig::MaxEntities];
    bool Active[EngineConfig::MaxEntities];
    bool HasLastPosition[EngineConfig::MaxEntities];
};
...
### Core Movement Logic (`MovementSystem.cpp`)

```cpp
void MovementSystem::Update(
    Transform2D* transforms,
    const Vector2* velocities,
    const float* speeds,
    const bool* activeMask,
    int entityCount,
    float deltaTime)
{
...
### Transform2D (`Transform2D.h`)

```cpp
struct alignas(16) Transform2D
{
    Vector2 X, Y;   // Basis vectors (future rotation)
...
* SoA + contiguous memory → excellent cache behavior
...
* Use contiguous arrays and avoid pointer chasing
* Reuse temporary buffers (`std::vector` reserve or custom pool)
